...
class LocationModel {
    ...
    /**
     * Lấy provinces có products (với số lượng products)
     * @param {string} searchTerm - Từ khóa tìm kiếm (không bắt buộc)
     * @param {number} limit - Giới hạn số kết quả trả về
     * @returns {Array} Danh sách provinces có products
     */
    static async getProvincesWithProducts(searchTerm = null, limit = 50) {
        try {
            let query = `
                SELECT DISTINCT
                    p.ProvinceCode,
            ...
            if (searchTerm && searchTerm.trim().length > 0) {
                ...
                WHERE 
                    prov.Name LIKE ? OR 
                    prov.NameEn LIKE ? OR 
                    prov.FullName LIKE ? OR 
                    prov.FullNameEn LIKE ? OR 
                    prov.CodeName LIKE ?
                `;
                const searchPattern = `%${searchTerm.trim()}%`;
                queryParams.push(searchPattern, searchPattern, searchPattern, searchPattern, searchPattern);
            ...
            query += ` 
                GROUP BY p.ProvinceCode, prov.Name, prov.NameEn, prov.FullName, prov.FullNameEn, prov.CodeName
                ORDER BY ProductCount DESC, prov.Name ASC
                LIMIT ?
            `;
            queryParams.push(parseInt(limit));
    ...
    /**
     * Lấy districts có products theo province code
     * @param {string} provinceCode - Mã tỉnh/thành phố
     * @param {string} searchTerm - Từ khóa tìm kiếm (không bắt buộc)
     * @param {number} limit - Giới hạn số kết quả trả về
     * @returns {Array} Danh sách districts có products
     */
    static async getDistrictsWithProductsByProvince(provinceCode, searchTerm = null, limit = 100) {
        try {
            let query = `
                SELECT DISTINCT
                    d.DistrictCode,
                ...
                FROM Districts d
                LEFT JOIN Provinces p ON d.ProvinceCode = p.ProvinceCode
                JOIN Products prod ON d.DistrictCode = prod.DistrictCode
                WHERE d.ProvinceCode = ?
            `;
            ...
            query += ` 
                GROUP BY d.DistrictCode, d.Name, d.NameEn, d.FullName, d.FullNameEn, d.CodeName, d.ProvinceCode, p.Name, p.NameEn
                ORDER BY ProductCount DESC, d.Name ASC
                LIMIT ?
            `;
            queryParams.push(parseInt(limit));
            ...
            console.error('Error in getDistrictsWithProductsByProvince:', error);
            throw error;
        }
    }
