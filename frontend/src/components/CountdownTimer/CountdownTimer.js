...
const CountdownTimer = ({ details, onEnded }) => {
  const endTs = useMemo(() => {
  ...
  const calc = () => {
    if (!endTs || details.status === 'ended') return { done: true, days: 0, hours: 0, minutes: 0, seconds: 0 };
  ...
  useEffect(() => {
    if (timeLeft.done && details.status !== 'ended') {
      onEnded && onEnded();
    }
  }, [timeLeft.done, onEnded]);
